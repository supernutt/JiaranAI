...
class RetryCategory:
    ...
    MANIM_API_ERROR = "MANIM_API_ERROR"  # Outdated Manim API usage (get_graph with color, etc.)
    RECURRING_API_ERROR = "RECURRING_API_ERROR"  # Same API error occurring multiple times
    COLOR_DEFINITION_ERROR = "COLOR_DEFINITION_ERROR"  # Using undefined color constants
...
def categorize_error(error_message: str) -> str:
    ...
    # Add specific detection for color definition errors
    elif "nameerror" in error_trace_lower and any(color in error_trace_lower for color in ["_dark", "_light", "color", "green_dark", "blue_dark", "red_dark", "yellow_dark"]):
        return RetryCategory.COLOR_DEFINITION_ERROR
    # Add specific detection for Manim API errors like get_graph with color parameter
    elif ("unexpected keyword argument" in error_trace_lower and "color" in error_trace_lower) or \
         ("get_graph" in error_trace_lower and "unexpected keyword argument" in error_trace_lower):
        return RetryCategory.MANIM_API_ERROR
...
3. Plotting and Graphs:
   - Axes(x_range=[start, end, step], y_range=[start, end, step], ...)
   ...
   - axes.get_axis_labels(x_label, y_label)  # For axis labels ONLY (not tick labels)
4. Colors:
   - RED, GREEN, BLUE, YELLOW, WHITE, BLACK, GRAY, PINK, PURPLE, ORANGE
   - Variants: RED_A, RED_B, RED_C, RED_D, RED_E (same for all basic colors)
   - Special: DARK_BLUE, DARK_BROWN, DARK_GRAY, LIGHT_GRAY, PURE_RED, PURE_GREEN, PURE_BLUE
   - NEVER use colors like GREEN_DARK, BLUE_DARK, RED_DARK - use _D variants instead (e.g., GREEN_D)
5. Animation Methods:
   - Create(mobject), Write(mobject), FadeIn(mobject), FadeOut(mobject), Transform(mobject, target_mobject), ReplacementTransform(mobject, target_mobject), GrowFromCenter(mobject), mobject.animate.method()
6. Positioning and Movement:
   - Directions: UP, DOWN, LEFT, RIGHT, UL, UR, DL, DR (NO UP_RIGHT, DOWN_LEFT, etc.)
   - mobject.move_to(point), mobject.shift(vector), mobject.to_edge(direction), mobject.next_to(other_mobject, direction, buff=float)
7. Group Operations:
   - VGroup(*mobjects)
8. Scene Control:
   - self.play(*animations, run_time=float), self.wait(duration=float), self.add(*mobjects)
9. Mathematical Constants:
   - PI, np.pi

**IMPORTANT API RULES:**
- Only use axes.get_axis_labels(x_label, y_label) for axis labels (not for tick labels). Pass exactly two arguments, both MathTex or Text objects.
- NEVER use get_x_axis_labels() or get_y_axis_labels() (these do not exist).
- Only use valid directions: UP, DOWN, LEFT, RIGHT, UL, UR, DL, DR. Do NOT use UP_RIGHT, DOWN_LEFT, etc.
- For plotting, use axes.plot(), not axes.get_graph().
- For graph labels, use MathTex().next_to(graph, direction).
- For colors, use only the listed constants and variants.
- Do not use any external assets, SVGs, or dependencies—use only built-in Manim features.
'''
    # --- END: Insert Manim asset catalog and explicit warnings ---

    retry_system_message = manim_asset_catalog + "\n\n"
    ...
    elif error_category == RetryCategory.COLOR_DEFINITION_ERROR:
        retry_system_message += "Hint: You are using an undefined color constant. The available Manim colors are:\n"
        retry_system_message += "Basic colors: RED, GREEN, BLUE, YELLOW, WHITE, BLACK, GRAY, PINK, PURPLE, ORANGE\n"
        retry_system_message += "Color variants: RED_A, RED_B, RED_C, RED_D, RED_E (same pattern for GREEN, BLUE, YELLOW)\n"
        retry_system_message += "Special colors: DARK_BLUE, DARK_BROWN, DARK_GRAY, LIGHT_GRAY, PURE_RED, PURE_GREEN, PURE_BLUE\n"
        retry_system_message += "DO NOT use colors like GREEN_DARK, BLUE_DARK, RED_DARK - these don't exist! Use GREEN_D, BLUE_D, RED_D instead.\n"
        retry_system_message += "If you need a darker shade, use the _D variant (e.g., GREEN_D) or create a custom color.\n"
    elif error_category == RetryCategory.MANIM_API_ERROR:
        retry_system_message += "Hint: You are using outdated Manim API syntax. DO NOT use get_graph() with color parameter. Instead use the current v0.18+ syntax:\n"
        retry_system_message += "INCORRECT: axes.get_graph(lambda x: np.sin(x), color=RED)\n"
        retry_system_message += "CORRECT: graph = axes.plot(lambda x: np.sin(x), x_range=[-4, 4], color=RED)\n"
        retry_system_message += "Always use axes.plot() instead of axes.get_graph() for plotting functions in current Manim.\n"
