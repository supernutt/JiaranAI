...
def generate_and_render_scene_task(
    ...
    try:
        ...
        # 1. Generate scene code from prompt (student question -> Manim prompt -> Manim code)
        # generate_manim_scene now returns a dict {"class_name": str, "code": str}
        try:
        ...
        # save_generated_scene might modify class_name if the initial one is bad
        try:
            ...
            logger.error(f"Task {task_id}: Generated code has syntax errors: {str(e)}")
            task_store[task_id]["status"] = TaskStatus.FAILURE
            task_store[task_id]["error_detail"] = f"Generated code has syntax errors: {str(e)}"
            task_store[task_id]["error_category"] = ErrorCategory.CODE_GENERATION_ERROR
            task_store[task_id]["message"] = "The generated code has syntax errors."
            task_store[task_id]["last_updated"] = time.time()
            save_task_to_file(task_id, task_store[task_id])
            return
        except Exception as e:
        ...
        # 3. Render the scene using Manim
        try:
            ...
            # generate_animation returns absolute path to the video file
            # Use a unique filename stem based on task_id or a timestamp to avoid collisions
            # For now, animation_generator uses a timestamped filename if not provided.
            ...
            logger.error(f"Task {task_id}: Manim rendering failed: {str(e)}")
            task_store[task_id]["status"] = TaskStatus.FAILURE
            task_store[task_id]["error_detail"] = f"Failed during video rendering: {str(e)}"
